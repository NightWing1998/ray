...
``real``  (when the environment is configured)
    ...
    and ``v`` = ``"r{id}"`` -- across several commits.
    ``python/ray/data/tests/datasource/load_azure_uc_test_table.py`` provisions
    exactly that against a SQL warehouse; run it before these tests.
...
class _RealAzureBackend:
    ...
    def seed(self):
        ...
        # well outside what a test should stand up. See
        # `load_azure_uc_test_table.py`.
        pass
